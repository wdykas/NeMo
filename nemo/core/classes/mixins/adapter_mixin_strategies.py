...
class AddAdapterStrategy(AbstractAdapterStrategy):
    """
    An implementation of addition of an adapter module with its input.
    Supports stochastic depth regularization.
    """
...
@dataclass
class AddAdapterStrategyConfig:
    stochastic_depth: float = 0.0
    _target_: str = "{0}.{1}".format(
        AddAdapterStrategy.__module__, AddAdapterStrategy.__name__
    )  # mandatory field