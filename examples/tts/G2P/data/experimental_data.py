...
def prepare_hifi_tts(manifest,
	...
	os.makedirs(output_dir, exist_ok=True)
	with open(manifest, "r") as f_in, open(
			f"{output_dir}/{os.path.basename(manifest)}", "w") as f_out:
		for line in tqdm(f_in):
			line = json.loads(line)
...
if __name__ == "__main__":
	...
	with open(f"{CMU_DICT_SPLITS_DIR}/train.txt", "w") as train_f, open(f"{CMU_DICT_SPLITS_DIR}/dev.txt", "w") as dev_f, open(
		f"{CMU_DICT_SPLITS_DIR}/test.txt", "w"
	) as test_f:
	...
	prepare_hifi_tts(f"{BASE_DIR}/all_hifi_tts.json", output_dir=TRAINING_DATA_DIR, phoneme_dict=f"{CMU_DICT_SPLITS_DIR}/train.txt")
