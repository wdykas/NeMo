...
from nemo.collections.asr.models import EncDecRNNTBPEModel, TSEncDecRNNTBPEModel
from nemo.core.config import hydra_runner
from nemo.utils import logging
from nemo.utils.exp_manager import exp_manager


@hydra_runner(config_path="../conf/conformer/", config_name="conformer_transducer_bpe_ts")
def main(cfg):
    ...
    if cfg.do_training:
        asr_model = TSEncDecRNNTBPEModel(cfg=cfg.model, trainer=trainer)

        # Initialize the weights of the model from another model, if provided via config
        # asr_model.maybe_init_from_pretrained_checkpoint(cfg)

        checkpoint = EncDecRNNTBPEModel.restore_from(
        ...
        asr_model = TSEncDecRNNTBPEModel.restore_from(
            cfg.nemo_checkpoint_path, map_location=torch.device('cpu'), strict=False
        )
